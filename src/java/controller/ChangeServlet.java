...
public class ChangeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String rpass = request.getParameter("rpass"); // confirm password

        if (npass.equals(rpass)) {
            // Passwords match, continue with the change
            DAO dao = new DAO();
            Account account = dao.checkAccount1(user, opass);

            if (account == null) {
                // Old password is incorrect
                String ms = "Old password is wrong";
                ...
                // Set the updated account in the session
            ...
            String ms = "New password and confirm password do not match";
            request.setAttribute("ms", ms);
            request.getRequestDispatcher("change.jsp").forward(request, response);
        }
    }
